...
func (a *Adapter) LoadFilteredPolicy(model model.Model, filter interface{}) error {
	...
	if err := a.db.Scopes(a.filterQuery(filterValue)).Find(&lines).Error; err != nil {
		return err
	}
...
func (a *Adapter) filterQuery(filter Filter) func(db *gorm.DB) *gorm.DB {
	return func(db *gorm.DB) *gorm.DB {
...
func (a *Adapter) AddPolicies(_ string, ptype string, rules [][]string) error {
	return a.db.Transaction(func(tx *gorm.DB) error {
		...
		for _, rule := range rules {
			line := a.savePolicyLine(ptype, rule)
			casbinRules = append(casbinRules, line)
		}
...
func (a *Adapter) UpdatePolicy(ptype string, oldRule, newPolicy []string) error {
