...
func (mw *GinJWTMiddleware) GetClaimsFromJWT(c *gin.Context) (MapClaims, error) {
	...
	if mw.SendAuthorization {
		if v, ok := c.Get("JWT_TOKEN"); ok {
			c.Header("x-token", mw.TokenHeadName+" "+v.(string))
		}
	}
...
// LoginBaseHandler can be used by clients to get a jwt token.(兼容旧版本)
// Payload needs to be json in the form of {"username": "USERNAME", "password": "PASSWORD"}.
// Reply will be of the form {"token": "TOKEN"}.
// GenerateCaptcha 获取验证码(兼容老版本)
// @Summary 获取验证码
// @Description 获取验证码
// @Tags 登陆
// @Success 200 {string} string "{"code": 200, "expire": "2019-08-07T12:45:48+08:00", "token": ".eyJleHAiOjE1NjUxNTMxNDgsImlkIjoiYWRtaW4iLCJvcmlnX2lhdCI6MTU2NTE0OTU0OH0.-zvzHvbg0A" }"
// @Router /base/login [post]
func (mw *GinJWTMiddleware) LoginBaseHandler(c *gin.Context) {
	if mw.Authenticator == nil {
		mw.unauthorized(c, http.StatusInternalServerError, mw.HTTPStatusMessageFunc(ErrMissingAuthenticatorFunc, c))
		return
	}
	...
	}{expire.Format(time.RFC3339), "Bearer " + tokenString}
	c.JSON(http.StatusOK, gin.H{
		"code": 0,
		"data": s,
		"msg":  "登录成功",
